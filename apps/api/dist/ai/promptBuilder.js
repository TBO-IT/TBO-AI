import { QuestionValidationError } from "./questionTypes.js";
...
// ─── Helpers ─────────────────────────────────────────────────────────────────
function formatColumns(semanticLayer) {
    return semanticLayer.allColumns
        .map(col => `  "${col.column_name}"`)
        .join("\n");
}
function formatRelevantMetrics(semanticLayer, parsedQuestion) {
    ...
    const relevantMetrics = semanticLayer.metrics.filter(m => {
        ...
        return relevantKeys.some(k => k === metricKeyFromName ||
...
function formatDimensions(semanticLayer, parsedQuestion) {
    const relevantDims = parsedQuestion?.dimensions.length
        ? parsedQuestion.dimensions
        : semanticLayer.dimensions;
    const defs = semanticLayer.businessDefinitions.filter(d => relevantDims.includes(d.name));
...
function formatTimeContext(semanticLayer, parsedQuestion) {
...
function formatFilters(parsedQuestion) {
    if (!parsedQuestion?.filters.length)
        return "";
...
function formatIntent(parsedQuestion) {
    if (!parsedQuestion)
        return "";
    const intentHints = {
 ...
 */
export function buildPrompt(question, semanticLayer) {
