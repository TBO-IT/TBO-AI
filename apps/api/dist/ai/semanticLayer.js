...
function detectTimeColumns(schema) {
    const allTime = [];
    schema.forEach(col => {
...
function mapDimensions(schema) {
    ...
    const norm = (s) => s.toLowerCase().trim().replace(/[\s_]+/g, "_");
    schema.forEach(col => {
...
function resolveMetrics(datasetType) {
...
function resolveBusinessDefinitions(dimensions) {
    return dimensions.map(dim => {
        const concept = BUSINESS_KNOWLEDGE.concepts[dim];
        return {
            name: dim,
 ...
 */
export function buildSemanticLayer(schema) {
