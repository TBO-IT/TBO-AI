 ...
 */
export async function callClaudeWithStructuredOutput(prompt, toolSchema, requestType, systemPrompt = "You are an Executive Analytics Copilot.", temperature = 0.0) {
    ...
    for (let attempt = 1; attempt <= MAX_RETRIES; attempt++) {
        try {
            ...
            // Track usage
            if (response.usage) {
                await recordUsage(model, requestType, response.usage.input_tokens, response.usage.output_tokens);
            }
