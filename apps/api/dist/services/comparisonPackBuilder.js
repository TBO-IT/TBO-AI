...
async function profileEntity(entityName, dimension, physicalCol, metricFormula, metricName, csvPath) {
 ...
 */
export async function buildComparisonPack(left, right, dimension, physicalCol, semanticLayer, csvPath) {
...
export function formatComparisonNarrative(pack) {
    const lines = [
        ...
        "",
        "### Entity Profiles",
        ...pack.entities.map(e => `- **${e.entity}**: ${e.volume} rows | ${e.winRate.toFixed(1)}% ${pack.metricName} | Weakest APW: ${e.worstApw} | Weakest Destination: ${e.worstDestination} | Weakness: ${e.biggestWeakness} | Opportunity: ${e.biggestOpportunity}`),
        "",
        "### Focus Areas",
