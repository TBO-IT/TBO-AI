...
const METRIC_VERSION = crypto
    ...
    .digest("hex")
    .slice(0, 8);
function buildCacheKey(datasetType, question) {
    return [
        "sql_cache",
        SQL_CACHE_VERSION,
        datasetType,
...
export async function getCachedSql(datasetType, normalizedQuestion) {
...
export async function setCachedSql(datasetType, normalizedQuestion, sql) {
    //const key = `sql_cache:v${METRIC_VERSION}:${datasetType}:${normalizedQuestion}`;
    const key = buildCacheKey(datasetType, normalizedQuestion);
    try {
...
export async function deleteCachedSql(datasetType, normalizedQuestion) {
    const key = `sql_cache:v${METRIC_VERSION}:${datasetType}:${normalizedQuestion}`;
    try {
        await redis.del(key);
