export var DecisionIntent;
(function (DecisionIntent) {
    DecisionIntent["EXPLAIN"] = "EXPLAIN";
    DecisionIntent["IMPROVE"] = "IMPROVE";
    DecisionIntent["FIX"] = "FIX";
    DecisionIntent["COMPETE"] = "COMPETE";
    DecisionIntent["PRIORITIZE"] = "PRIORITIZE";
    DecisionIntent["EXPAND"] = "EXPAND";
    DecisionIntent["PROTECT"] = "PROTECT";
})(DecisionIntent || (DecisionIntent = {}));
export var TargetPolarity;
(function (TargetPolarity) {
    TargetPolarity["POSITIVE"] = "POSITIVE";
    TargetPolarity["NEGATIVE"] = "NEGATIVE";
    TargetPolarity["RISK"] = "RISK";
})(TargetPolarity || (TargetPolarity = {}));
function calculateResourceAllocationScore(entry, impactScore, intent) {
    ...
    const concentrationPenalty = entry.volumeSharePct >= 25 ? entry.volumeSharePct * 0.45 :
        entry.volumeSharePct >= 15 ? entry.volumeSharePct * 0.25 :
            entry.volumeSharePct >= 10 ? entry.volumeSharePct * 0.15 :
                0;
    const baseScore = (recoverableImpact * 0.45) +
...
function derivePolarity(impactScore, intent) {
...
function buildSelectionRationale(entry, impactScore, resourceAllocationScore, polarity, intent) {
...
export function calculateActionabilityTargets(allContributors, intent = DecisionIntent.EXPLAIN, competitorContext, metricName = "performance metric") {
    if (!allContributors?.length) {
        console.warn("[ACTIONABILITY_ENGINE] allContributors is empty or undefined");
        return [];
    }
    const targets = allContributors.map(({ entry, type }) => {
