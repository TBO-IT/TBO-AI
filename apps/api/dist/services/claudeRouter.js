...
const HAIKU_OPS = new Set([
    "EXECUTIVE_SUMMARY",
    "NARRATIVE_GENERATION",
    "EXPLAIN_RESULTS",
    "TREND_EXPLANATION"
]);
const SONNET_OPS = new Set([
    "RECOMMENDATIONS",
    "STRATEGIC_ANALYSIS",
 ...
 */
export function routeClaude(analyticsRoute, operation, hasValidPack = false) {
 ...
 */
export function selectClaudeTier(operation) {
    if (HAIKU_OPS.has(operation))
        return "HAIKU";
    if (SONNET_OPS.has(operation))
        return "SONNET";
    return "NONE";
}
// ─── Internal ─────────────────────────────────────────────────────────────────
function decide(tier, operation, maxTokens, reason) {
    const decision = {
    ...
    console.log(`[CLAUDE_ROUTER] shouldCall=${decision.shouldCallClaude} | tier=${tier} | ` +
