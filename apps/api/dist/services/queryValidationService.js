 ...
 */
export function validateQueryPreExecution(analysis, semanticLayer) {
    const errors = [];
    const suggestions = [];
    // 1. Validate metrics exist in the semantic layer
    for (const metric of analysis.metrics) {
        const found = semanticLayer.metrics.some(m => m.name.toLowerCase() === metric.toLowerCase());
        if (!found) {
    ...
    for (const dim of analysis.dimensions) {
        const found = semanticLayer.dimensions.some(d => d.toLowerCase() === dim.toLowerCase());
        if (!found) {
    ...
    const timeFilters = analysis.filters.filter(f => ["month", "quarter", "year"].includes(f.dimension));
    ...
    if (analysis.intent === "COMPARISON") {
        const comparisonFilters = analysis.filters.filter(f => !["month", "quarter", "year", "time", "_entity"].includes(f.dimension));
    ...
    if (analysis.intent === "ROOT_CAUSE") {
        if (timeFilters.length < 2) {
            ...
            suggestions.push(`Root cause analysis works best with two time periods. ` +
    ...
    for (const [dim, values] of filtersByDim.entries()) {
        ...
        if (values.size > 2 && !["month", "quarter", "year"].includes(dim)) {
            suggestions.push(`Multiple values detected for "${dim}": ${[...values].join(", ")}. ` +
 ...
 */
export function validateQueryResults(results, routeType, question) {
    const errors = [];
    const suggestions = [];
    if (!results || results.length === 0) {
        errors.push("No data found matching your query.");
        if (routeType === "ROOT_CAUSE") {
            suggestions.push("Root cause analysis requires data in both time periods. " +
                "Check that your date range contains data.");
        }
        else if (routeType === "COMPARISON") {
            suggestions.push("The entities you're comparing may not exist in the dataset. " +
    ...
    if (results.length > 0) {
        const allNull = results.every(row => Object.values(row).every(v => v === null || v === undefined));
        if (allNull) {
