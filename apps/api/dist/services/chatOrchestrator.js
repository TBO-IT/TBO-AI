...
import { setCachedSql } from "./queryCacheService.js";
import { getCachedNarrative, setCachedNarrative } from "./narrativeCacheService.js";
import { ClaudeOutputSchemas } from "../ai/outputSchemas.js";
...
import { classifyResponseSource, detectNarrativeRequest, detectRecommendationRequest, isExecutivePriorityQuestion } from "./claudeRequestDetector.js";
...
import { generateRecommendations, generateRecommendationsStream } from "./recommendationGenerator.js";
import { detectCompetitorContext } from "./competitorDetector.js";
import { startTimer, logCache, logClaude } from "./analyticsLogger.js";
...
export class ChatOrchestrator {
    static async execute(datasetId, userId, question, opts) {
        ...
        try {
            ...
            console.log(`[SEMANTIC_LAYER] type=${semanticLayer.datasetType} | ` +
            ...
            competitorContext = detectCompetitorContext(question, metadata, mergedFilters.map(f => ({ dimension: f.dimension, value: f.value })));
            if (competitorContext) {
                // Check if we already have a supplier filter for this competitor
                const alreadyFiltered = mergedFilters.some(f => (f.dimension === "supplier" || f.dimension === "thirdparty") &&
                ...
                if (!alreadyFiltered) {
                    ...
                    console.log(`[COMPETITOR_FILTER_CREATED]\n` +
                        `thirdparty=${competitorContext.competitorName}`);
                }
                else {
                    console.log(`[COMPETITOR_FILTER_SKIPPED] competitor=${competitorContext.competitorName} | ` +
                        `reason=already_present`);
                }
            }
            else if (parsedQuestion.intent === "COMPETITOR_STRATEGY") {
                console.warn(`[COMPETITOR_FALLBACK_GLOBAL] Competitive intent detected but no thirdparty match — ` +
            ...
            if (cachedSql) {
                ...
                if ((isRecommendation || isNarrative) && !packRoutes.has(routeType) && routeType !== "LLM" && (!routeType.includes("COMPETITOR") || shouldOverrideCompetitor)) {
                    console.log(`[PRE_ROUTER] ${isRecommendation ? "Recommendation" : "Narrative"} query detected — ` +
                ...
                // dimensions, then wraps the results in the Root Cause Pack Builder.
                    ...
                    let availableDims = ["hotel", "chain", "supplier", "apw"].filter(dim => semanticLayer.dimensions.some(d => d.toLowerCase() === dim.toLowerCase()));
                    if (parsedQuestion.dimensions.length > 0) {
                        const requestedDims = availableDims.filter(dim => parsedQuestion.dimensions.some(pd => pd.toLowerCase() === dim.toLowerCase()));
                        if (requestedDims.length > 0) {
                            availableDims = requestedDims;
                        }
                    }
                    const rootCauseSqls = [];
                    const rootCauseExplanations = [];
                ...
                if (routeType === "LLM") {
                    try {
                        ...
                        const generated = await callClaudeWithStructuredOutput(prompt, ClaudeOutputSchemas.generatedQuery, "SQL_GENERATION", "You are a Senior DuckDB SQL Expert for a travel analytics platform.");
                        sql = generated.sql;
            ...
            if (routeType === "ROOT_CAUSE") {
                ...
                queryResultsList = await Promise.all(sqlStatements.map(async (statement) => {
                    try {
            ...
            if (routeType === "EXECUTIVE_PRIORITY") {
                const priorityResult = await runExecutivePriorityPipeline(question, parsedQuestion, semanticLayer, tempPath, competitorContext || undefined);
                ...
                comparisonPackResult = await buildComparisonPack(entities.left, entities.right, entities.dimension, entities.physicalCol, semanticLayer, tempPath);
                rootCausePack = {
                ...
                const drilldowns = await executeEntityDrilldown(rootCausePack.primaryTarget, parsedQuestion, semanticLayer, tempPath, undefined, competitorContext ?? undefined);
                rootCausePack.recommendations = generateAttributedRecommendations(rootCausePack.primaryTarget, drilldowns, competitorContext || undefined);
                ...
                let compPrimaryTarget = undefined;
                let drilldowns = [];
                let recommendations = [];
            ...
            if ((routeType === "ROOT_CAUSE" || routeType === "EXECUTIVE_PRIORITY" || routeType === "COMPETITOR_STRATEGY") && rootCausePack && executivePack) {
                ...
                claudeInputPack = buildClaudeInputPack(question, rootCausePack, executivePack, competitorContext ? competitorContext.competitorName : undefined);
                if (competitorContext) {
                    console.log(`[COMPETITOR_RCA] competitor=${competitorContext.competitorName} | ` +
            ...
            let narrative = "";
            const cachedNarrative = await getCachedNarrative(datasetId, question.toLowerCase().trim(), sql, responseSource);
            if (cachedNarrative) {
            ...
            console.log(`[PIPELINE_COMPLETE] route=${routeType} | responseSource=${responseSource} | ` +
            ...
            console.log(`[RETURN_PATH] MAIN_RETURN | responseSource=${responseSource} | ` +
 ...
 * No Claude involved.
 */
function buildDeterministicNarrative(question, queryResults, insights) {
 ...
 */
function buildRecommendationNarrative(recommendations, pack) {
...
function buildExecutivePriorityNarrative(executivePack) {
    const target = executivePack.primaryTarget;
    const decisionBrief = executivePack.decisionBrief;
    const drivers = (executivePack.topDrivers ?? []).slice(0, 3).map((d) => d.name).join(", ");
    const actions = (executivePack.topActions ?? []).slice(0, 3).map((action, index) => {
    ...
    const alternatives = (decisionBrief?.alternatives ?? []).slice(0, 3).map((alt) => {
    ...
    return [
        ...
        ``,
        `**Recommended Actions:**`,
        ...(actions ? actions.split("\n") : (executivePack.recommendedFocusAreas ?? []).slice(0, 3).map((a) => `- ${a}`)),
        ``,
        `**Leadership Message:** ${executivePack.leadershipMessage || executivePack.keyTakeaway || ""}`,
        `**Action Verb:** ${polarityAction}`
    ].filter(Boolean).join("\n");
}
function validateExecutivePriorityNarrative(executivePack, narrative, question) {
    const target = executivePack?.primaryTarget;
    if (!target) {
