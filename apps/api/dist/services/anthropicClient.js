...
function getClient() {
    if (!_client) {
        ...
        if (!apiKey) {
            throw new AnthropicClientError("ANTHROPIC_API_KEY is not set.", "CONFIG_ERROR");
        }
...
function getModel(tier) {
    ...
    if (!model) {
        throw new AnthropicClientError(`Model not configured for tier ${tier}. Set CLAUDE_${tier}_MODEL env var.`, "CONFIG_ERROR");
    }
    return model;
}
 ...
 */
export async function generateText(prompt, systemPrompt, tier, maxTokens = 1500, temperature = 0.1) {
    ...
    for (let attempt = 1; attempt <= MAX_RETRIES; attempt++) {
        try {
            ...
            throw new AnthropicClientError(`Claude failed after ${attempt} attempts: ${err.message}`, code, status);
        }
    }
 ...
 */
export async function generateNarrativeText(prompt, systemPrompt) {
 ...
 */
export async function generateNarrativeTextStream(prompt, systemPrompt, onToken, abortSignal, maxTokens = 1200, temperature = 0.1) {
    validatePrompt(prompt);
    ...
    for await (const event of response) {
        ...
        const deltaText = anyEvent?.delta?.text;
 ...
 */
export async function generateRecommendationText(prompt, systemPrompt) {
 ...
 */
export async function generateRecommendationTextStream(prompt, systemPrompt, onToken, abortSignal, maxTokens = 1500, temperature = 0.2) {
    validatePrompt(prompt);
...
function validatePrompt(prompt) {
    ...
    if (prompt.length > MAX_PROMPT_CHARS) {
        throw new AnthropicClientError(`Prompt too long: ${prompt.length} > ${MAX_PROMPT_CHARS} chars.`, "PROMPT_TOO_LONG");
    }
