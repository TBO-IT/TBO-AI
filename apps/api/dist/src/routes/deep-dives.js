...
router.get("/hotel/:id", requireAuth(), currentUser, async (req, res) => {
    try {
        const { id } = req.params;
        ...
        let dataset = null;
        if (datasetId !== "demo") {
            dataset = await getDataset(datasetId, req.user.id);
            if (!dataset) {
                return res.status(404).json({ error: "Dataset not found" });
            }
        }
        const hotelName = decodeURIComponent(id);
        let winRateVal = 42.5;
        ...
        if (datasetId !== "demo" && dataset?.storagePath) {
            ...
            const totalRes = await executeQuery(`SELECT COUNT(*) as total FROM data_table`, localPath);
            ...
            `;
            const metricsRes = await executeQuery(sql, localPath);
            ...
            const supplierSql = `
                ...
                LIMIT 5
            `;
            const suppliersRes = await executeQuery(supplierSql, localPath);
...
router.get("/supplier/:id", requireAuth(), currentUser, async (req, res) => {
    try {
        const { id } = req.params;
        const { datasetId } = req.query;
        if (!datasetId) {
        ...
        if (datasetId !== "demo" && dataset?.storagePath) {
            ...
            const hotelSql = `
                ...
                LIMIT 5
            `;
            const hotelsRes = await executeQuery(hotelSql, localPath);
...
router.get("/chain/:id", requireAuth(), currentUser, async (req, res) => {
    try {
        const { id } = req.params;
        const { datasetId } = req.query;
        if (!datasetId) {
        ...
        if (datasetId !== "demo" && dataset?.storagePath) {
            ...
            const propSql = `
                ...
                LIMIT 5
            `;
            const propsRes = await executeQuery(propSql, localPath);
