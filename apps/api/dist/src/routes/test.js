...
import { buildDatasetContext } from "../services/schemaService.js";
...
router.post("/prompt", async (req, res) => {
    try {
        ...
        if (!dataset.storagePath) {
            return res.status(400).json({
                error: "Dataset has no storage path"
            });
        }
        const localCsvPath = await downloadDataset(dataset.storagePath);
        const context = await buildDatasetContext(localCsvPath);
        const semanticLayer = buildSemanticLayer(context.schema);
