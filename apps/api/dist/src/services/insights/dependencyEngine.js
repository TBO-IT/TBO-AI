...
// ───────────────────────────────────────────────────────────────────────────────
export function detectDependencies(drivers) {
    const insights = [];
    if (drivers.length === 0) {
        return insights;
    }
    ...
    const totalContribution = drivers.reduce((sum, d) => sum + Math.abs(d.contributionPct), 0);
    const top3Contribution = top3.reduce((sum, d) => sum + Math.abs(d.contributionPct), 0);
    const top3SharePct = totalContribution > 0
        ? (top3Contribution / totalContribution * 100).toFixed(0)
        : "0";
    const top3VolumeShare = top3.reduce((sum, d) => sum + d.volumeSharePct, 0).toFixed(1);
    insights.push({
        entities: top3Names,
        explanation: `${top3Names.join(", ")} account for ${top3SharePct}% of total performance ` +
    ...
    if (topDriver && Math.abs(topDriver.contributionPct) >= 30) {
        insights.push({
            ...
            explanation: `${topDriver.name} alone accounts for ${Math.abs(topDriver.contributionPct).toFixed(1)}% ` +
    ...
    if (positiveCount > 0 && negativeCount > 0) {
        ...
        insights.push({
            ...
            explanation: `Performance is driven by ${ratio} contributors. ` +
