...
function allKnownValues(metadata) {
    return [
...
function entityExistsInMetadata(name, metadata) {
 ...
 */
export function shouldValidateEntityExistence(question) {
    const q = question.toLowerCase();
    return (/what should we do about\b/.test(q) ||
        /\babout [a-z0-9]/i.test(question) ||
        /recommendations? for\b/.test(q) ||
        /what should we do for\b/.test(q));
}
export function validateEntityExistence(filters, metadata) {
    ...
    for (const filter of entityFilters) {
        ...
        if (filter.dimension === "destination" && metadata.destinations.some(d => normalize(d) === normalize(name)))
            continue;
        if (filter.dimension === "supplier" && metadata.suppliers.some(s => normalize(s) === normalize(name)))
            continue;
        if (filter.dimension === "thirdparty" && (metadata.thirdParties ?? []).some(t => normalize(t) === normalize(name)))
            continue;
        if (filter.dimension === "chain" && metadata.chains.some(c => normalize(c) === normalize(name)))
            continue;
        if (filter.dimension === "hotel" && metadata.hotels.some(h => normalize(h) === normalize(name)))
            continue;
