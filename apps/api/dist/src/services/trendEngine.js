...
function detectGranularity(question) {
    ...
    for (const granularity of ["QUARTER", "YEAR", "MONTH"]) {
 ...
 */
function resolvePhysicalColumn(canonicalKey, semanticLayer) {
 ...
 */
function buildPeriodExpression(dateCol, granularity) {
    const granLabel = granularity.toLowerCase();
 ...
 */
function buildTrendWhereClause(analysis, semanticLayer) {
    ...
    const typedFilters = analysis.filters.filter(f => !TIME_DIMENSIONS.has(f.dimension) && f.dimension !== "_entity");
 ...
 */
export function generateTrendSql(analysis, semanticLayer) {
    // ── 1. Resolve metric ──────────────────────────────────────────────────────
    ...
    console.log(`[TrendEngine] Metric: ${metric.name} | Granularity: ${granularity} | ` +
    ...
    const filterCount = analysis.filters.filter(f => !["month", "year", "quarter", "time"].includes(f.dimension)).length;
    const filterNote = filterCount > 0
        ? ` with ${filterCount} filter${filterCount > 1 ? "s" : ""} applied`
        : "";
    const explanation = `${granLabel} trend of ${metric.name}${dimLabel}${filterNote}, ` +
