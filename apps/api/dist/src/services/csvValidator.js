...
import { getBestMatchingSchema, getDisplayHeaderName, normalizeHeader, resolveDatasetTypeByHeaders, } from "../config/datasetSchema.js";
...
function ensureConsistentColumns(rows, columnCount) {
    for (let i = 1; i < rows.length; i++) {
        if (rows[i].length !== columnCount) {
            throw new CsvValidationError(`Malformed CSV: row ${i + 1} has ${rows[i].length} columns, expected ${columnCount}.`);
        }
    }
}
export async function validateCsv(filePath) {
    ...
    const normalizedHeaderToOriginal = new Map();
    ...
    for (let i = 1; i <= maxRow; i++) {
        ...
        for (const [col, allowedValues] of enumColumns) {
            ...
            if (!isAllowed) {
                throw new CsvValidationError(`Invalid enum value in ${getDisplayHeaderName(col)} at row ${rowNum}. Allowed: ${allowedValues.join(", ")}.`);
            }
        }
    }
    return {
