 ...
 */
export function checkClaudeAllowed(operation, routeType) {
 ...
 */
export function estimateClaudeCost(estimatedInputTokens, estimatedOutputTokens) {
 ...
 */
export function recordClaudeCost(inputTokens, outputTokens) {
    const { estimatedCostUsd } = estimateClaudeCost(inputTokens, outputTokens);
    sessionTotalCost += estimatedCostUsd;
    sessionCallCount++;
    console.log(`[GUARDRAIL] Claude call #${sessionCallCount} cost: $${estimatedCostUsd.toFixed(4)} | ` +
