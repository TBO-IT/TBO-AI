...
async function analyzeCompetitiveness(conn, src) {
    const overviewSql = `
        SELECT
            COUNT(*) as rowCount,
    ...
    `;
    const [overview, apwBreakdown, chainPerformance, supplierPerformance] = await Promise.all([
        runQuery(conn, overviewSql),
        runQuery(conn, apwSql),
        runQuery(conn, chainSql),
        runQuery(conn, supplierSql),
    ]);
    return {
...
async function analyzeConversion(conn, src) {
    ...
    const [overview, topCities, topHotels] = await Promise.all([
        runQuery(conn, overviewSql),
        runQuery(conn, citySql),
        runQuery(conn, hotelSql),
    ]);
    return {
        rowCount: n(overview[0].rowCount),
...
async function analyzeGeneric(conn, src) {
    const rows = await runQuery(conn, `SELECT COUNT(*) as rowCount FROM ${src}`);
    return { rowCount: n(rows[0].rowCount) };
}
// ─── Schema detection ─────────────────────────────────────────────────────────
async function detectType(conn, src) {
    const cols = await runQuery(conn, `DESCRIBE SELECT * FROM ${src}`);
...
export async function analyzeCsv(filePath) {
    ...
    try {
        ...
        return { datasetType, ...typeSpecific };
    }
    finally {
        conn.close();
