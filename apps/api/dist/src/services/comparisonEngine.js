 ...
 */
export function extractComparisonEntities(analysis, semanticLayer) {
    ...
    console.log(`[ComparisonEngine] COMPARISON_DEBUG\n` +
    ...
    console.warn(`[ComparisonEngine] COMPARISON_DEBUG — FAILED\n` +
...
function buildPeriodSides(analysis, semanticLayer, filters) {
 ...
 */
function buildInClauseSql(entities, metric, sharedWhere) {
 ...
 */
function buildCTESql(sideA, sideB, metric, sharedWhere) {
 ...
 */
function buildEntityIlikeCondition(value, semanticLayer) {
    const safe = value.replace(/'/g, "''");
    const stringCols = semanticLayer.allColumns.filter(c => c.column_type.toUpperCase().includes("VARCHAR") ||
 ...
 */
export function generateComparisonSql(analysis, semanticLayer) {
    // ── 1. Resolve metric ──────────────────────────────────────────────────────
    ...
    const entities = extractComparisonEntities({ ...analysis, filters: deduped }, semanticLayer);
    if (entities && entities.dimension !== "_entity") {
        // Shared filters: everything except the comparison dimension
        const sharedFilters = deduped.filter(f => f.dimension !== entities.dimension && !TIME_DIMS.has(f.dimension));
        ...
        const explanation = `Comparing ${metric.name} for "${entities.left}" vs "${entities.right}" ` +
            `by ${entities.dimension}.`;
        console.log(`[ComparisonEngine] IN-clause SQL generated | ` +
    ...
    if (periodSides) {
        ...
        const usedTimeDim = ["quarter", "month", "year"].find(td => deduped.filter(f => f.dimension === td).length >= 2);
        const sharedFilters = deduped.filter(f => f.dimension !== usedTimeDim && !TIME_DIMS.has(f.dimension) && f.dimension !== "_entity");
        const sharedWhere = buildWhereClause(sharedFilters, schemaColumns);
    ...
    if (entities && entities.dimension === "_entity") {
        ...
        const sharedFilters = deduped.filter(f => f.dimension !== "_entity" && !TIME_DIMS.has(f.dimension));
        const sharedWhere = buildWhereClause(sharedFilters, schemaColumns);
        const sql = buildCTESql(sideA, sideB, metric, sharedWhere);
        const explanation = `Comparing ${metric.name} for "${entities.left}" vs "${entities.right}".`;
        console.log(`[ComparisonEngine] ILIKE CTE SQL generated | ` +
            `"${entities.left}" vs "${entities.right}"`);
        return { sql, explanation };
    }
    // ── 4. Failure ─────────────────────────────────────────────────────────────
    console.warn(`[ComparisonEngine] COMPARISON_DEBUG — FINAL FAILURE\n` +
        `  METRIC:  ${metric.name}\n` +
