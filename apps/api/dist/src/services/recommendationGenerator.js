...
import { generateRecommendationText } from "./anthropicClient.js";
 ...
 */
export function buildRecommendationPrompt(pack) {
 ...
 */
export async function generateRecommendations(pack) {
    // 1. Safety gate
    try {
...
export async function generateRecommendationsStream(pack, opts) {
    ...
    try {
        ...
        const result = await generateRecommendationTextStream(prompt, systemPrompt, onToken, opts.abortSignal);
...
function parseClaudeRecommendations(text) {
    const recs = [];
...
export function buildDeterministicRecommendations(pack) {
    const recs = [];
