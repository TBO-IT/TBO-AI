...
function buildContradictionPack() {
    const sl = buildSemanticLayer(MOCK_SCHEMA);
    const mockResults = [[
...
describe("Claude Input Contract", () => {
    ...
    it("rejects a pack containing SQL patterns", () => {
        ...
        pack.metricName = "SELECT * FROM data_table";
        assert.throws(() => assertClaudeInputSafe(pack), /SECURITY VIOLATION/);
    });
    it("rejects a pack containing file paths", () => {
        const pack = buildTestPack();
        pack.metricName = "C:\\\\Users\\\\test\\.csv";
        assert.throws(() => assertClaudeInputSafe(pack), /SECURITY VIOLATION/);
    });
