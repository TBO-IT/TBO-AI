...
describe("csvValidator", () => {
    it("accepts a valid CSV", async () => {
        const filePath = await writeTempFile("valid.csv", [
    ...
    it("rejects missing columns", async () => {
        const filePath = await writeTempFile("missing-column.csv", [
        ...
        await assert.rejects(validateCsv(filePath), (error) => {
            assert.ok(error instanceof CsvValidationError);
            assert.match(error.message, /Missing columns/);
            assert.match(error.message, /suppliername/i);
            return true;
        });
    });
    it("rejects duplicate headers", async () => {
        const filePath = await writeTempFile("duplicate-header.csv", [
            "destination,destination,suppliername",
            "goa,goa,supplier-a",
        ].join("\n"));
        await assert.rejects(validateCsv(filePath), (error) => {
            assert.ok(error instanceof CsvValidationError);
            assert.match(error.message, /Duplicate header/);
            return true;
        });
    });
    it("rejects binary files", async () => {
        ...
        await assert.rejects(validateCsv(filePath), (error) => {
            assert.ok(error instanceof CsvValidationError);
            assert.match(error.message, /Binary file detected/);
            return true;
        });
    });
    it("rejects invalid dates", async () => {
        const filePath = await writeTempFile("bad-date.csv", [
            "Competitive Status,price_diff_perc,apw_bucket_new,tbo_chainname,suppliername,thirdparty_price,tbo_price,scraped_date",
            "Winning,12.5,high,chain-a,supplier-a,100,90,2026-13-01",
        ].join("\n"));
        await assert.rejects(validateCsv(filePath), (error) => {
            assert.ok(error instanceof CsvValidationError);
            assert.match(error.message, /Invalid date value/);
            return true;
        });
    });
    it("rejects invalid numeric values", async () => {
        const filePath = await writeTempFile("bad-number.csv", [
            "Competitive Status,price_diff_perc,apw_bucket_new,tbo_chainname,suppliername,thirdparty_price,tbo_price,scraped_date",
            "Winning,not-a-number,high,chain-a,supplier-a,100,90,2026-01-15",
        ].join("\n"));
        await assert.rejects(validateCsv(filePath), (error) => {
            assert.ok(error instanceof CsvValidationError);
            assert.match(error.message, /Invalid numeric value/);
            return true;
        });
    });
    it("rejects invalid enum values", async () => {
        const filePath = await writeTempFile("bad-enum.csv", [
            "Competitive Status,price_diff_perc,apw_bucket_new,tbo_chainname,suppliername,thirdparty_price,tbo_price,scraped_date",
            "Unknown,12.5,high,chain-a,supplier-a,100,90,2026-01-15",
        ].join("\n"));
        await assert.rejects(validateCsv(filePath), (error) => {
            assert.ok(error instanceof CsvValidationError);
            assert.match(error.message, /Invalid enum value/);
            return true;
        });
    });
    it("rejects empty files", async () => {
        ...
        await assert.rejects(validateCsv(filePath), (error) => {
            assert.ok(error instanceof CsvValidationError);
            assert.match(error.message, /CSV is empty/);
            return true;
        });
    });
    it("rejects malformed CSV", async () => {
        const filePath = await writeTempFile("malformed.csv", [
            "Competitive Status,price_diff_perc,apw_bucket_new,tbo_chainname,suppliername,thirdparty_price,tbo_price,scraped_date",
...
describe("upload gate", () => {
    it("does not call DuckDB for invalid CSVs", async () => {
        const filePath = await writeTempFile("invalid.csv", [
            "Competitive Status,price_diff_perc,apw_bucket_new,tbo_chainname,suppliername,thirdparty_price,tbo_price,scraped_date",
            "Unknown,12.5,high,chain-a,supplier-a,100,90,2026-01-15",
        ].join("\n"));
