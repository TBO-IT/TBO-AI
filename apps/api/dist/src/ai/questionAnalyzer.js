import { METRIC_SYNONYMS, DIMENSION_SYNONYMS, INTENT_SIGNALS, TIME_SIGNALS } from "./questionKnowledge.js";
...
// ─── Metric Extraction ────────────────────────────────────────────────────────
function extractMetrics(normalizedQuestion) {
    const found = new Set();
...
function extractDimensions(normalizedQuestion) {
    const found = new Set();
 ...
 */
function extractApwBucketFilters(question) {
    const filters = [];
    const apwDef = DIMENSION_REGISTRY["apw"];
    if (!apwDef?.validValues)
        return filters;
 ...
 */
function extractStatusFilters(question) {
    const filters = [];
    const statusDef = DIMENSION_REGISTRY["competitive_status"];
    if (!statusDef?.validValues)
        return filters;
...
function extractNamedEntityFilters(originalQuestion) {
    const filters = [];
    ...
    const flush = () => {
        ...
        const isKnownTerm = METRIC_SYNONYMS.some(e => e.synonyms.some(s => s === normalized)) ||
 ...
 */
function extractTimeFilters(normalizedQuestion) {
    const filters = [];
    ...
    for (const signal of sorted) {
        if (containsPhrase(tempText, signal)) {
            ...
            const filter = buildTimeFilter(signal);
            if (filter) {
                filters.push(filter);
            }
 ...
 */
function extractAllFilters(originalQuestion, normalizedQuestion) {
    ...
    const entityFilters = extractNamedEntityFilters(originalQuestion).filter(f => !recognizedValues.has(String(f.value).toLowerCase()));
    return [...apwFilters, ...statusFilters, ...entityFilters];
}
// ─── Time Reference Extraction ────────────────────────────────────────────────
function extractTimeReferences(normalizedQuestion) {
    const found = [];
    const sorted = [...TIME_SIGNALS].sort((a, b) => b.length - a.length);
    let tempText = normalizedQuestion;
    for (const signal of sorted) {
...
function detectIntent(normalizedQuestion) {
    ...
    const hasPartial = (signals) => signals.some(s => q.includes(s));
    // ── Helper: phrase match (whole-word / whole-phrase) ───────────────────────
    const hasPhrase = (signals) => signals.some(s => containsPhrase(q, s));
    // ── 1. TREND ──────────────────────────────────────────────────────────────
...
export function analyzeQuestion(question) {
    ...
    const filters = [
        ...extractAllFilters(question, normalizedQuestion),
        ...extractTimeFilters(question)
    ];
    console.log("TIME FILTERS:", extractTimeFilters(question));
    ...
    if (isExecutiveQuestion(question)) {
        const hasTimeComparison = Array.from(TIME_COMPARISON_TERMS).some(term => normalizedQuestion.includes(term));
    ...
    console.log(`[ANALYSIS] Intent: ${intent} | Metrics: [${metrics.join(",")}] | ` +
