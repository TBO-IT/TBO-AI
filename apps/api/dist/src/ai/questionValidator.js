 ...
 */
export function validateQuestion(parsedQuestion, semanticLayer) {
    ...
    const hasTimeColumn = !!semanticLayer.primaryTimeDimension ||
    ...
    for (const filter of parsedQuestion.filters) {
        ...
        if (filter.dimension === "_entity" || filter.dimension === "time")
            continue;
        ...
        if (!isValidFilterValue(filter.dimension, String(filter.value))) {
            ...
            errors.push(`Invalid filter value '${filter.value}' for dimension '${dimDef.label}'. ` +
    ...
    console.log(`[VALIDATOR] valid=${errors.length === 0} | ` +
        `VALIDATED_FILTERS=[${parsedQuestion.filters.map(f => `${f.dimension}=${f.value}`).join(",")}]`);
    if (errors.length > 0) {
        console.error(`[VALIDATION_REJECT] question="${parsedQuestion.originalQuestion}" | ` +
