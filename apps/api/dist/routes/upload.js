...
function logUploadValidationFailure(params) {
...
router.post("/", upload.single("file"), currentUser, asyncHandler(async (req, res) => {
    if (!req.file) {
    ...
    try {
        ...
        const dataset = await createDataset(req.user.id, req.file.originalname, storagePath);
        ...
        const summary = await analyzeCsv(filePath);
        const redisKey = `dataset:${dataset.id}`;
        await redis.set(redisKey, summary);
        await markCompleted(dataset.id, summary.rowCount, redisKey);
        return res.json({
