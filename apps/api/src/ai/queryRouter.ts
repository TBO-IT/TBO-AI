 ...
 */
const CONTRIBUTION_SIGNALS: string[] = [
 ...
 */
const ROOT_CAUSE_SIGNALS: string[] = [
    "why did",
    "why does",
    "why is ",
    "why are",
 ...
 */
const ROOT_CAUSE_LEADING_WORDS: string[] = [
    "why ",
    "cause",
    "reason"
];
 ...
 */
export function isTrendQuestion(question: string, intent: QuestionIntent): boolean {
    if (intent === "TREND") return true;
 ...
 */
export function isComparisonQuestion(question: string, intent: QuestionIntent): boolean {
 ...
 */
export function isContributionQuestion(question: string): boolean {
    const lower = question.toLowerCase();
 ...
 */
export function isRootCauseQuestion(question: string, intent: QuestionIntent): boolean {
    if (intent === "ROOT_CAUSE") return true;
    const lower = question.toLowerCase();

    // Strong multi-word causal phrases (highest confidence)
    if (ROOT_CAUSE_SIGNALS.some(s => lower.includes(s))) return true;
    ...
    if (ROOT_CAUSE_LEADING_WORDS.some(w => trimmed.startsWith(w))) return true;
