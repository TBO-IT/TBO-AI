...
export interface HeaderNormalizationConfig {
...
export interface DatasetContract {
    ...
    DATE_COLUMNS: string[];
    ENUM_COLUMNS: Record<string, string[]>;
    OPTIONAL_COLUMNS: string[];
    HEADER_NORMALIZATION: HeaderNormalizationConfig;
    CLASSIFICATION_KEYWORDS: string[];
}

export const DEFAULT_HEADER_NORMALIZATION: HeaderNormalizationConfig = {
    trimWhitespace: true,
    collapseRepeatedSpaces: true,
    caseInsensitive: true,
};

export const DATASET_SCHEMAS: Record<DatasetType.COMPETITIVENESS | DatasetType.CONVERSION, DatasetContract> = {
...
export function getDisplayHeaderName(rawHeader: string): string {
    const cleaned = normalizeWhitespace(rawHeader);
    const lowered = cleaned.toLowerCase();
    return HEADER_ALIASES[lowered] ?? cleaned;
}

export function resolveDatasetTypeByHeaders(headers: string[]): DatasetType {
    const normalizedHeaders = headers.map(normalizeHeader);

    let bestType: DatasetType = DatasetType.UNKNOWN;
    let bestScore = -1;

    for (const [datasetType, schema] of Object.entries(DATASET_SCHEMAS) as Array<[DatasetType, DatasetContract]>) {
...
export function getBestMatchingSchema(headers: string[]): {
    datasetType: DatasetType.COMPETITIVENESS | DatasetType.CONVERSION;
    schema: DatasetContract;
} {
    const inferredType = resolveDatasetTypeByHeaders(headers);
    if (inferredType !== DatasetType.UNKNOWN) {
        return {
            datasetType: inferredType as DatasetType.COMPETITIVENESS | DatasetType.CONVERSION,
            schema: DATASET_SCHEMAS[inferredType as DatasetType.COMPETITIVENESS | DatasetType.CONVERSION],
        };
    }

    const normalizedHeaders = new Set(headers.map(normalizeHeader));
    const candidates = Object.entries(DATASET_SCHEMAS) as Array<[DatasetType.COMPETITIVENESS | DatasetType.CONVERSION, DatasetContract]>;
