import { RootCausePack, ContributorEntry } from "./RootCausePackBuilder.js";

/**
 * Validates a generated Root Cause Pack for structural and logical errors.
 */
export function validateRootCausePack(pack: RootCausePack): string[] {
    const errors: string[] = [];

    // 1. Check for contradictions
    if (pack.contradictionDetected) {
        errors.push(
            `Contradiction: User expected ${pack.expectedDirection}, ` +
            `but metric actually went ${pack.metricChange?.direction}.`
        );
    }

    // 2. Validate entity names are not numeric
    const isNumericString = (s: string) => !isNaN(Number(s)) && s.trim() !== "";
    
    const checkNames = (entries: ContributorEntry[], label: string) => {
 ...
 */
function validateContributionAnalysis(
