...
function resolveContributionDimension(
    ...
    for (const dimKey of analysis.dimensions) {
        if (TIME_DIMENSIONS.has(dimKey)) continue;
 ...
 */
function detectPeriodPair(
    analysis: QuestionAnalysis,
    dateCol: string
): { current: PeriodSpec; prior: PeriodSpec } | null {
...
interface PeriodSpec {
    timeDim: "quarter" | "month" | "year";
    value: number;
    dateCol: string;
}

function buildPeriodCondition(spec: PeriodSpec): string {
...
function periodLabel(spec: PeriodSpec): string {
    if (spec.timeDim === "quarter") return `Q${spec.value}`;
    if (spec.timeDim === "month")   return `Month ${spec.value}`;
    return `${spec.value}`;
}
 ...
 */
function buildSinglePeriodSql(
    dim: { physicalCol: string; label: string },
...
export function generateContributionSql(
    ...
    console.log(
        `[ContributionEngine] Metric: ${metric.name} | Dimension: ${dim.canonicalKey} | ` +
    ...
    if (periods) {
        ...
        console.log(
            `[ContributionEngine] Mode: two-period | ` +
