...
function buildOpportunity(
    ...
    return {
        title:
            `${driver.name} expansion opportunity`,
        ...
        impactScore:
            Number(driver.impactScore.toFixed(2)),

        explanation:
            `${driver.name} improved by ` +
            `${driver.metricDelta.toFixed(2)} points ` +
            `while representing ` +
            `${driver.volumeSharePct.toFixed(1)}% of total volume.`,

        recommendedAction:
            buildAction(category)
    };
}

function buildMomentumOpportunity(
    driver: PrioritizedInsight
): ExecutiveOpportunity {

    return {
        title:
            `${driver.name} momentum acceleration opportunity`,
        ...
        impactScore:
            Number((driver.impactScore * 1.2).toFixed(2)),

        explanation:
            `${driver.name} is demonstrating strong positive momentum ` +
            `with a ${driver.metricDelta.toFixed(2)} point improvement ` +
            `and meaningful contribution to overall performance.`,

        recommendedAction:
            "Prioritize investment and investigate opportunities to scale this performance across other segments."
    };
}

function buildScaleOpportunity(
    driver: PrioritizedInsight
): ExecutiveOpportunity {

    return {
        title:
            `${driver.name} scale-up opportunity`,
        ...
        explanation:
            `${driver.name} is performing strongly despite limited volume share (${driver.volumeSharePct.toFixed(1)}%).`,

        recommendedAction:
            "Increase exposure and test whether strong performance can be sustained at larger scale."
    };
}
...
export function detectOpportunities(
    ...
    for (const driver of priorityDrivers) {
        ...
        const meaningfulUpside =
            driver.volumeSharePct >= 5 ||
            driver.metricDelta >= 3;

        if (meaningfulUpside) {
            opportunities.push(
                buildOpportunity(driver)
            );
        }
        ...
        if (
            ...
            opportunities.push(
                buildMomentumOpportunity(driver)
            );
        }
    ...
    const deduped = opportunities.filter(
        (opp, index, self) =>
            index ===
            self.findIndex(
                o =>
