...
export interface Scenario {
    type: "BEST_CASE" | "BASE_CASE" | "WORST_CASE";
    description: string;
}
...
export function generateScenarios(
    ...
    if (topOpp && topRisk) {
        scenarios.push({
            type: "BEST_CASE",
            description:
                `If ${topOpp.affectedEntity} momentum is replicated across additional segments ` +
        ...
        scenarios.push({
            type: "BEST_CASE",
            description:
                `If ${topOpp.affectedEntity} outperformance is sustained and scaled, ` +
        ...
        scenarios.push({
            type: "BEST_CASE",
            description:
                `If current positive trends continue without new headwinds, ` +
    ...
    if (isFlat) {
        scenarios.push({
            type: "BASE_CASE",
            description:
                `Current ${metricName} stability persists, but underlying volatility ` +
        ...
        scenarios.push({
            type: "BASE_CASE",
            description:
                `${metricName} continues its current decline trajectory at a moderate pace. ` +
        ...
        scenarios.push({
            type: "BASE_CASE",
            description:
                `${metricName} improvement continues at the current rate, ` +
        ...
        scenarios.push({
            type: "BASE_CASE",
            description:
                `Current performance trajectory continues with no material change.`
        });
    }
    ...
    if (topOpp && topRisk) {
        scenarios.push({
            type: "WORST_CASE",
            description:
                `If ${topOpp.affectedEntity} momentum slows while ${topRisk.affectedEntity} ` +
        ...
        scenarios.push({
            type: "WORST_CASE",
            description:
                `If ${topRisk.affectedEntity} deterioration accelerates without mitigation, ` +
        ...
        scenarios.push({
            type: "WORST_CASE",
            description:
                `If unexpected headwinds emerge in key segments, ` +
