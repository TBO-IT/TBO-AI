...
export function detectTradeoffs(
    ...
    for (const opp of opportunities) {
        ...
        if (matchingRisk && !seen.has(opp.affectedEntity)) {
            ...
            tradeoffs.push({
                ...
                explanation:
                    `${opp.affectedEntity} is simultaneously our strongest growth driver ` +
    ...
    for (const driver of drivers) {
        if (
            ...
            tradeoffs.push({
                ...
                explanation:
                    `${driver.name} growth is improving performance, but increasing ` +
    ...
    for (const risk of risks) {
        if (risk.category === "CONCENTRATION" && !seen.has(risk.affectedEntity)) {
            const isPositive = drivers.find(
                d => d.name === risk.affectedEntity && d.direction === "POSITIVE"
            );
            if (isPositive) {
                ...
                tradeoffs.push({
                    ...
                    explanation:
                        `${risk.affectedEntity} is a strong performer, but high volume ` +
